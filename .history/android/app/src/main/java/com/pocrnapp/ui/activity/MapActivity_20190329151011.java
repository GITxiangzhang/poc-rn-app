...
import com.pocrnapp.MainActivity;
import com.pocrnapp.R;
import com.pocrnapp.ui.adapter.SearchResultAdapter;
import com.pocrnapp.ui.view.SegmentedGroup;
    ...
    private void init() {
        ...
        aMap.setOnCameraChangeListener(new AMap.OnCameraChangeListener() {
            ...
            public void onCameraChangeFinish(CameraPosition cameraPosition) {
                ...
                searchLatlonPoint = new LatLonPoint(cameraPosition.target.latitude,
...
//        Log.i("MY", "geoAddress"+ searchLatlonPoint.toString());
        ...
        if (searchLatlonPoint != null) {
            RegeocodeQuery query =
                    new RegeocodeQuery(searchLatlonPoint, 200, GeocodeSearch.AMAP);//
...
//        Log.i("MY", "doSearchQuery");
        currentPage = 0;
        query = new PoiSearch.Query(searchKey, searchType, "");// 第一个参数表示搜索字符串，第二个参数表示poi
        // 搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
    ...
    public void onRegeocodeSearched(RegeocodeResult result, int rCode) {
        ...
        if (rCode == AMapException.CODE_AMAP_SUCCESS) {
            if (result != null && result.getRegeocodeAddress() != null
                ...
                String address =
                        result.getRegeocodeAddress().getProvince() + result.getRegeocodeAddress().getCity() + result.getRegeocodeAddress().getDistrict() + result.getRegeocodeAddress().getTownship();
    ...
    AdapterView.OnItemClickListener onItemClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            if (position != searchResultAdapter.getSelectedPosition()) {
                ...
                LatLng curLatlng = new LatLng(poiItem.getLatLonPoint().getLatitude(),
    ...
    private void searchPoi(Tip result) {
        ...
        inputSearchKey = result.getName();//getAddress(); // + result.getRegeocodeAddress()
        // .getCity() + result.getRegeocodeAddress().getDistrict() + result.getRegeocodeAddress()
        // .getTownship();
